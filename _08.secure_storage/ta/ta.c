...
TEE_Result save_aes_key(uint8_t *key, size_t key_size) {
    ...
    TEE_CreatePersistentObject(TEE_STORAGE_PRIVATE, key_id, strlen(key_id),
...
TEE_Result load_aes_key(uint8_t *key, size_t key_size) {
    ...
    TEE_OpenPersistentObject(TEE_STORAGE_PRIVATE, key_id, strlen(key_id),
                                   TEE_DATA_FLAG_ACCESS_READ, &object);
        return res;
    }

    TEE_ReadObjectData(object, key, key_size, &read_size);
    TEE_CloseObject(object);
